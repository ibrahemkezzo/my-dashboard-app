...
@section('breadcrumbs')
    <x-dashboard.dashboard-breadcrumb :breadcrumbs="[
        ...
        ['label' => __('dashboard.add_services'), 'url' => ''],
    ]" :pageName="__('dashboard.add_services')" />
@endsection
...
@section('content')
    ...
    <div class="card">
        <div class="card-header">
            <h3>{{ __('dashboard.add_services') }} - {{ __('Step 2') }}</h3>
        </div>
        <div class="card-body">
            @if ($errors->any())
            ...
            @endif
            <form action="{{ route('dashboard.salons.store.step2',$salon) }}" method="POST" enctype="multipart/form-data">
                @csrf
