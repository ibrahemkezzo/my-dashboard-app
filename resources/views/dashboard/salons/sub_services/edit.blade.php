...
@section('content')
    ...
    <div class="card">
        ...
        <div class="card-body">
            ...
            @endif
            <form action="{{ route('dashboard.salons.sub-services.update', [$salon, $subService]) }}" method="POST" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                <div class="row mb-3">
                    <div class="col-md-6">
                        ...
                        <select name="service_id" class="form-control main-service-select" required>
                            ...
                            @foreach($allServices as $service)
                                <option value="{{ $service->id }}" {{ old('service_id', $subService->subService->service_id) == $service->id ? 'selected' : '' }}>{{ $service->name }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="col-md-6">
                        ...
                        <select name="sub_service_id" class="form-control sub-service-select" required>
                            ...
                            @foreach($subServices as $ss)
                                @if($ss->service_id == $subService->subService->service_id)
                                    <option value="{{ $ss->id }}" {{ old('sub_service_id', $subService->sub_service_id) == $ss->id ? 'selected' : '' }}>{{ $ss->name }}</option>
                                @endif
                            @endforeach
                        </select>
                    </div>
                ...
                <div class="row mb-3">
                    <div class="col-md-3">
                        ...
                        <input type="number" step="0.01" class="form-control" name="price" value="{{ old('price', $subService->price) }}">
                    </div>
                    <div class="col-md-3">
                        <label class="form-label">{{ __('dashboard.duration') }} ({{ __('dashboard.minutes') }})</label>
                        <input type="number" class="form-control" name="duration" value="{{ old('duration', $subService->duration) }}">
                    </div>
                    <div class="col-md-3">
                        <label class="form-label">{{ __('dashboard.status') }}</label>
                        <select class="form-control" name="status">
                            <option value="1" {{ old('status', $subService->status) == 1 ? 'selected' : '' }}>{{ __('dashboard.active') }}</option>
                            <option value="0" {{ old('status', $subService->status) == 0 ? 'selected' : '' }}>{{ __('dashboard.inactive') }}</option>
                        </select>
                    </div>
                    <div class="col-md-3">
                            ...
                            <div class="row mt-2">
                                @foreach($subService->media as $media)
                                    <div class="col-md-2 text-center mb-2">
                                        <img src="{{ asset('storage/'.$media->path) }}" alt="service" style="width:80px;height:80px;object-fit:cover;border-radius:8px;">
                                    </div>
                                @endforeach
                            </div>
                        </div>
                ...
                <div class="d-flex gap-2">
                    ...
                    <a href="{{ route('dashboard.salons.sub-services.show', [$salon, $subService]) }}" class="btn btn-outline-primary">{{ __('dashboard.cancel') }}</a>
                </div>
            </form>
        </div>
    </div>
