    ...
    <div class="container-fluid">
        <div class="card">
            <div class="card-header d-flex justify-content-between align-items-center flex-wrap">
                <h5 class="mb-0">{{ __('dashboard.users') }}</h5>
                <a href="{{ route('dashboard.users.create') }}" class="btn btn-primary">
                    <i class="fa fa-plus"></i> {{ __('dashboard.create_user') }}
                </a>
            </div>
            <div class="card-body">
                <div class="card-header d-flex justify-content-between align-items-center flex-wrap">
                    ...
                    <form action="{{ route('dashboard.users.index') }}" method="GET" class="search-container">
                        ...
                        <input class="search-input" type="search" name="search" value="{{ $filters['search'] ?? '' }}"
                    ...
                    <div class="d-flex flex-wrap gap-2 align-items-center">
                        <form method="GET" action="{{ route('dashboard.users.index') }}"
                            ...
                            <div class="form-group mb-0">
                                <select name="roles[]" class="form-control" style="min-width: 90px;">
                                    <option value="">{{ __('dashboard.all_roles') }}</option>
                                    @foreach ($roles as $id => $name)
                                        <option value="{{ $name }}"
                                            {{ in_array($name, $filters['roles'] ?? []) ? 'selected' : '' }}>
                                            {{ $name }}</option>
                                    @endforeach
                                </select>
                            </div>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        ...
                                        <a href="#" class="text-red-500 hover:text-red-700"
                                            ...
                                            onclick="event.preventDefault(); return confirm('{{ __('dashboard.are_you_sure_delete_user') }}') && document.getElementById('destroy-form-{{ $user->id }}').submit();">
